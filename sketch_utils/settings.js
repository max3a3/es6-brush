...
co.polygon = function (a, b, c, s) {
  ...
  function fu_y() {
    return (y - Math.sin(angle + (step * n)) * rad * scale.Y);
  }
...
co.star = function (a, b, c, s) {
  var T = !isNaN(s) ? 'marquee' : 'shape', o = co.constrain(a, b, s), scale = co.scale(a, b, s);
  var rad = Math.sqrt(Math.pow(a.X - o.X, 2) + Math.pow(a.Y - o.Y, 2)) / 2, rad2 = rad / vars['slope_' + T],
    sides = vars['sides_' + T];
  ...
  function fu1(v) {
    return ((v == 'X') ? (x + Math.cos(angle + (step * n) - halfStep) * rad * scale.X) : (y - Math.sin(angle + (step * n) - halfStep) * rad * scale.Y));
  }

  function fu2(v) {
    return ((v == 'X') ? (x + Math.cos(angle + (step * n)) * rad2 * scale.X) : (y - Math.sin(angle + (step * n)) * rad2 * scale.Y));
  }

  var n = 0;
  c.moveTo(fu2('X'), fu2('Y'));

  for (var n = 1; n <= sides; n++) {

    c.lineTo(fu1('X'), fu1('Y'));
    c.lineTo(fu2('X'), fu2('Y'));
